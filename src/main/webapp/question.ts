...
module Question
{
	export class Plugin
	{
		...
		new_question: NewQuestion.Plugin;
		unit: string;
		type: string;
		dom: JQuery;
		private no = 0;

		init(unit: string, type: string)
		{
			var thethis = this;
			this.unit = unit;
			this.type = type;
			...
			$.get('question.html', function (data)
			{
				...
				importmodal.find('.id_save').click(function()
				{
					...
					plus['wait'].emit();
					var form = <HTMLFormElement> importmodal.find('.id_importform')[0];
