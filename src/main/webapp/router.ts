...
module router
{
	...
	class Hrteam
	{
		...
		init()
		{
			...
			if (!plus['wait']) plus['wait'] = new plus.DelayedEmitter();
			if (!plus['ready']) plus['ready'] = new plus.DelayedEmitter();
			if (!plus['change']) plus['change'] = new plus.DelayedEmitter();
			var thethis = this;
	...
	export function route()
	{
		...
		isLoggedin(function (il)
		{
			...
			{
				var app = new App();
				var requestlist:RequestList.Plugin;
				var contentLoaded = new plus.Emitter();
				plus.bar([contentLoaded, app.onReady], ()=>
				{
