...
module CVUpload
{
	export class Plugin
	{
		...
		serialize(obj, prefix = undefined)
		{
			var thethis = this;
			var str = [];
			for (var p in obj)
		...
		init(requestid:string)
		{
			...
			$.get('cvupload.html', function (data)
			{
				...
				t['submitcv'] = function ()
				{
					...
					if (thethis.emailTb.val() == "" || thethis.nameTb.val() == "") missing = true;
					...
					{
						var form:HTMLFormElement = <HTMLFormElement>thethis.dom.find('.id_docform')[0];
					...
					plus.upload('HRWeb/cv/upload?' + thethis.serialize(data),
